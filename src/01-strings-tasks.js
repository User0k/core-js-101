...
function removeFirstOccurrences(str, value) {
  const redExp = new RegExp(value);
  return str.replace(redExp, '');
}
...
function getRectangleString(width, height) {
  ...
  const bodyFargment = height > 2
    ? `│${' '.repeat(width - 2)}│\n`
    : '';
  return head + bodyFargment.repeat(height - 2) + foot;
}
